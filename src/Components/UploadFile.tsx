...
import { useUploadVideoMutation } from "../Redux/api/videoApi.ts"; // импортируем хук RTK Query
...
const UploadFile = () => {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, targetPath: string) => {
        ...
        try {
            ...
            // Бэкенд может вернуть строку или объект { videoUrl }
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full w-full">
                ...
                {!isLoading ? (
                    <>
                        <div className="flex-1 bg-white p-8 rounded-xl shadow-lg mt-30 flex flex-col items-center border border-gray-200">
                            ...
                            <ul className="text-left text-sm text-gray-600 space-y-2 mb-8 w-full">
                                <li className="flex items-center"><span className="text-green-500 mr-2">•</span>Распознавание речи</li>
                                <li className="flex items-center"><span className="text-green-500 mr-2">•</span>Автоматический перевод на татарский</li>
                                <li className="flex items-center"><span className="text-green-500 mr-2">•</span>Создание субтитров</li>
                            </ul>
                            <input
                                type="file"
                                accept="video/*"
                        ...
                        <div className="flex-1 bg-white p-8 rounded-xl shadow-lg flex flex-col mt-30 items-center border border-gray-200">
                            ...
                            <ul className="text-left text-sm text-gray-600 space-y-2 mb-8 w-full">
                                <li className="flex items-center"><span className="text-blue-500 mr-2">•</span>Субтитры на татарском языке</li>
                                <li className="flex items-center"><span className="text-blue-500 mr-2">•</span>Генерация сводки по видео</li>
                                <li className="flex items-center"><span className="text-blue-500 mr-2">•</span>Полная транскрипция видео</li>
                            </ul>
                            <input
                                type="file"
                                accept="video/*"
                    ...
                    <div className="bg-white p-16 rounded-xl shadow-md text-center w-full max-w-lg border border-gray-200">
                        <div className="p-4">
                            ...
                            <p className="mt-2 text-sm text-gray-500">Идёт загрузка...</p>
                        </div>
                    </div>
                )}
            </div>
            ...
            {error && (
                <p className="text-red-600 mt-4">Ошибка загрузки файла</p>
            )}
        </div>
    );
};
